...
<?php   for ($index = 0; $index < ceil($models['found'] / $models['rows']); $index++) { ?>
				<span data-search="<?php echo $models['search'] ?? ''; ?>"<?php echo $models['start'] === $index * $models['rows'] ? ' style="font-weight:bold;"' : ''; ?> data-start="<?php echo $index * $models['rows']; ?>"><?php echo $index + 1; ?></span>&nbsp;
<?php   } ?>
